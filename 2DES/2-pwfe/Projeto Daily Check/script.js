document.addEventListener('DOMContentLoaded', function () {
  // Ativar a funcionalidade de alternância da barra de navegação
  ...
  formTemperatura.addEventListener('submit', function (event) {
    ...
    const inputLocal = document.querySelector('#temperatura form input')
    ...
    fetch(
      `https://api.openweathermap.org/data/2.5/weather?q=${local}&appid=e0800bb787cb97e908e0cd533766706d&units=metric`
    )
      .then(response => response.json())
      .then(data => {
        // Exibir a temperatura na página
        const temperaturaResultado = document.querySelector(
