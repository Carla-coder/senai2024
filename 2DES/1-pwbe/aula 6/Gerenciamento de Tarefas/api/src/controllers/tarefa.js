...
const addTarefa = (req, res) => {
  if (
    ...
    con.query(
      'INSERT INTO tarefa (descricao, data_vencimento, status, idUsuario) VALUES (?, ?, ?, ?)',
      [descricao, data_vencimento, status, idUsuario],
      (err, result) => {
        if (err) {
...
const getTarefas = (req, res) => {
  if (req.params.idTarefa != null) {
    con.query(
      'SELECT * FROM tarefa WHERE idTarefa =' + req.params.idTarefa,
      (err, result) => {
        if (err) {
...
const updateTarefa = (req, res) => {
  if (
    ...
    con.query(
      'UPDATE tarefa SET descricao = ?, data_vencimento = ?, status = ?, idUsuario = ? WHERE idTarefa = ?',
      [descricao, data_vencimento, status, idUsuario, id],
      (err, result) => {
        if (err) {
