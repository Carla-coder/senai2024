document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', e => {
      ...
      const Preço = parseFloat(document.getElementById('Preço').value)
  
      const item = { id, nome: nome, descricao: Descrição, preco: Preço }
    ...
    const addItemToList = item => {
      ...
      row.querySelector('.btn-edit').addEventListener('click', () => {
        ...
        document.getElementById('Preço').value = item.preco
    ...
    const updateItem = item => {
      ...
      itemsList.querySelectorAll('tr').forEach(row => {
        ...
        if (itemId === item.id) {
          ...
          row.cells[3].textContent = `R$ ${item.preco.toFixed(2)}`
        }
      })
    }
